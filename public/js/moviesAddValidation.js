...
window.onload = function () {
  ...
  $("title").addEventListener("blur", (e) => {
    ...
    if (e.target.value.length < 3)
      $("error-title").innerHTML = "Tan corto es el nombre de la pelicula !!";
    if (!e.target.value)
      $("error-title").innerHTML =
        "Te dije que escribas el nombre de la PEli!!";
  });
  ...
  $("rating").addEventListener("blur", (e) => {
    ...
    if (e.target.value > 10) $("error-rating").innerHTML = "Paraa!!";

    if (!e.target.value)
      $("error-rating").innerHTML = "No averiguaste el RATING???";
  });
